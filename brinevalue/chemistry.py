...
strength I <= 0.5 mol/L. Above that, SI numbers are not used for decisions;
pilot-grade work requires Pitzer / PHREEQC / Reaktoro (see docs/CHEMISTRY.md).
"""
...
class Brine:
    ...
    def validate(self):
        ...
        bad = [k for k, v in self.ions.items() if v is None or v < 0]
        if bad:
            raise ValueError(f"negative or missing concentrations: {bad}")
        return True
...
def ionic_balance(b: Brine):
    ...
    denom = cat + an
    err = abs(cat - an) / denom * 100 if denom else 0.0
    return {
        "cations_eq": round(cat, 5),
        "anions_eq": round(an, 5),
        "ionic_strength": round(b.ionic_strength(), 3),
        "balance_error_pct": round(err, 2),
        "acceptable": err < 10.0,
        "decision_grade": err <= 10.0,
        "reject_sample": err > 25.0,
    }
...
def scaling_risk(b: Brine):
    ...
    if reliable:
        ...
        # Do not drive decisions with invalid gammas (gamma can explode >1).
